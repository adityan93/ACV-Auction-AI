...
def val(net, dataset, criterion, max_iter=100):
    ...
    for i in range(min(max_iter, len(data_loader))):
        ...
        txt, ln = converter.encode(cpu_texts)
        utils.loadData(text, txt)
        utils.loadData(length, ln)
...
for epoch in range(opt.niter):
    ...
    while i < len(train_loader):
        ...
        loss_avg.add(cost)
        i += 1
        if epoch % opt.displayInterval == 0 and i == 1:
            print(cost)
            print('[%d/%d][%d/%d] Loss: %f' %
                  (epoch, opt.niter, i, len(train_loader), loss_avg.val()))
            loss_avg.reset()

        if epoch % opt.valInterval == 0 and i == 1:
            val(crnn, test_dataset, criterion)

        # do checkpointing
        if epoch % opt.saveInterval == 0 and i == 1:
            print("saving model...")
            torch.save(
